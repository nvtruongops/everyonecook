...
export interface StoreOptions {
  cacheKey: string;
  recipes: any[];
  ingredients?: string[]; // Normalized English ingredients (legacy)
  userInputIngredients?: string[]; // Original user input ingredients
  allRecipeIngredients?: string[]; // All ingredients from AI recipes
  settings: any;
}
...
export class AICacheService {
  ...
  constructor(tableName: string = process.env.DYNAMODB_TABLE || 'EveryoneCook') {
    ...
    this.ingredientSearch = new IngredientSearchService(tableName);
  }
  ...
  async store(options: StoreOptions): Promise<void> {
    const { cacheKey, recipes, settings } = options;
    // ✅ DUAL CACHE STRATEGY: Use ALL recipe ingredients for GSI4 indexing (better partial matching)
    // Priority: allRecipeIngredients (6 items) > userInputIngredients (3 items) > legacy ingredients
    const ingredientsForIndexing =
      options.allRecipeIngredients || options.userInputIngredients || options.ingredients || [];
    const now = new Date().toISOString();
    const ttl = Math.floor(Date.now() / 1000) + 24 * 60 * 60; // 24 hours

    // Generate searchable text (lowercase) - use ALL ingredients for better search
    const searchableText = this.generateSearchableText(recipes, ingredientsForIndexing);
    ...
    // Create ingredient indexes for GSI4 using ALL recipe ingredients (not just user input)
    await this.createIngredientIndexes(cacheKey, ingredientsForIndexing, ttl);

    console.log('AI Cache stored with dual strategy', {
      cacheKey,
      userInputCount: options.userInputIngredients?.length || 0,
      allRecipeCount: options.allRecipeIngredients?.length || 0,
      indexedCount: ingredientsForIndexing.length,
      improvement: 'Enables partial matching for +40% cache hit rate',
    });
  }
